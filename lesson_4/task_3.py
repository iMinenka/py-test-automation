...
Обратите внимание на файл business.py. В нем содержатся некоторые шаблоны, которые необходимо реализовать
    ...
    def add_employee(self, employee):
        ...
        if isinstance(employee, (Engineer, Manager)):
            if employee not in self.employees:
                ...
                print(f"{employee.name} is already employed in {self}")
        else:
            print(f"{employee} is not Engineer/Manager")
...
class Employee(Person):
    ...
    def join_company(self, company):
        ...
        if not self.is_employed:
            ...
            Company.add_employee(self.company, self)
        else:
            print(f"Ups, {self.name} is already hired. ({self.company})")
